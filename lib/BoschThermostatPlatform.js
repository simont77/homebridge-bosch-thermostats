...
class BoschThermostatPlatform {
    constructor(log, config, api) {
        ...
        api.on('didFinishLaunching', () => {
            ...
            boschThermostat.id = '0';
            boschThermostat.childDeviceIds ='0';
            
            boschThermostat.name = "test1";
            boschThermostat.serial = "11111111";
            ...
            //****************************************** */
            // temporary. To move in updateValues
            //****************************************** */
            setInterval(() => {
            for (var i = 0; i < this.thermostats.length; i++) {
                this.thermostats[i].thermostat.targetTemperature++;
                this.thermostats[i].thermostat.currentTemperature++;
                this.thermostats[i].thermostat.valvePosition++;
                this.thermostats[i].thermostat.humidityPercentage++;
                this.thermostats[i].ThermostatService.updateCharacteristic(this.Characteristic.CurrentTemperature, this.thermostats[i].thermostat.currentTemperature);
                this.thermostats[i].ThermostatService.updateCharacteristic(this.Characteristic.TargetTemperature, this.thermostats[i].thermostat.targetTemperature);
                this.thermostats[i].ThermostatService.updateCharacteristic(this.Characteristic.CurrentRelativeHumidity, this.thermostats[i].thermostat.humidityPercentage);
                this.thermostats[i].ThermostatService.updateCharacteristic("ValveOpening", this.thermostats[i].thermostat.valvePosition);
                this.thermostats[i].powerLoggingService.addEntry({time: Math.round(new Date().valueOf() / 1000), temp:this.thermostats[i].thermostat.currentTemperature, setTemp:this.thermostats[i].thermostat.targetTemperature, valvePosition:this.thermostats[i].thermostat.valvePosition, humidity:this.thermostats[i].thermostat.humidityPercentage});
            }
            },30000);
    ...
    establishConnection() {
        this.bshb = bosch_smart_home_bridge_1.BoschSmartHomeBridgeBuilder.builder()
            ...
            .build();
        this.bshb.pairIfNeeded('bshb', "homebridge", this.config.systemPassword,1000,1).pipe(operators_1.catchError(err => {
